...
export interface StoryEpisode {
  ...
  hook?: string;
  situation: string;
  malePerspective: Perspective;
  femalePerspective: Perspective;
  resolution: string;
  advice: string;
  nextTeaser?: string;
}
  ...
  return (
    <article className="max-w-3xl mx-auto my-12 bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-gray-100 transition-all">
      <header className="mb-12 text-center">
        <Badge variant="outline" className="mb-4 text-pink-600 border-pink-200 bg-pink-50 px-4 py-1">
      ...
      <div className="flex flex-col md:flex-row gap-8 mb-12 md:items-stretch">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <p className="text-gray-800 text-body-lg mb-6 whitespace-pre-line">{episode.femalePerspective.text}</p>
          <div className="bg-pink-50/50 rounded-2xl p-6 border border-pink-100 text-center flex-grow flex items-center justify-center min-h-[100px]">
            <p className="text-gray-600 italic text-body-sm">
              "{episode.femalePerspective.thoughtBubble}"
            </p>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <p className="text-gray-800 text-body-lg mb-6 whitespace-pre-line">{episode.malePerspective.text}</p>
          <div className="bg-blue-50/50 rounded-2xl p-6 border border-blue-100 text-center flex-grow flex items-center justify-center min-h-[100px]">
            <p className="text-gray-600 italic text-body-sm">
              "{episode.malePerspective.thoughtBubble}"
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 bg-gray-50 rounded-2xl p-8 text-center border border-gray-100">
        ...
        <div className="flex justify-center gap-4">
          <button
            onClick={() => setVoted('female')}
            className={`px-6 py-3 rounded-xl font-bold transition-all shadow-sm ${voted === 'female' ? 'bg-pink-600 text-white shadow-md transform scale-105' : 'bg-white text-gray-600 border border-gray-200 hover:border-pink-300 hover:text-pink-600'}`}
          >
            🙋‍♀️ 여자 (그녀의 마음이 이해돼요)
          </button>
          <button
            onClick={() => setVoted('male')}
            className={`px-6 py-3 rounded-xl font-bold transition-all shadow-sm ${voted === 'male' ? 'bg-blue-600 text-white shadow-md transform scale-105' : 'bg-white text-gray-600 border border-gray-200 hover:border-blue-300 hover:text-blue-600'}`}
          >
            🙋‍♂️ 남자 (당연히 이성적으로 생각해야지)
          </button>
        </div>
        {voted !== 'none' && (
          <p className="mt-6 text-body-sm text-green-600 font-bold animate-pulse">🎉 소중한 의견 감사합니다! 다음 뉴스레터 통계에 반영될 예정입니다.</p>
        )}
      </div>
      ...
      {episode.nextTeaser && (
        <div className="mt-12 pt-8 border-t-2 border-dashed border-gray-200 text-center">
          ...
          <p className="text-gray-600 font-serif text-body-lg italic">{episode.nextTeaser}</p>
        </div>
      )}
    </article>
  );
