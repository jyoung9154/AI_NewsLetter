...
export async function POST(request: Request) {
    console.log('[SEND BATCH] Starting daily personalized dispatch queue...');
    ...
    if (!smtpHost || !smtpUser || !smtpPass) {
        console.error('[SEND API] Missing SMTP credentials');
    ...
    for (const epNum of episodeNumbers) {
        ...
        console.log(`[SEND BATCH] Processing Episode ${epNum} for ${recipientEmails.length} recipients...`);
        ...
        if (epError || !episode) {
            console.warn(`[SEND BATCH] Episode ${epNum} not found in DB. Skipping this group.`);
            continue;
        }
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f3f4f6; margin: 0; padding: 20px; color: #1f2937;">
  <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; border: 1px solid #e5e7eb; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05);">
    ...
    <div style="text-align: center; padding: 32px; background-color: #f9fafb; border-top: 1px solid #e5e7eb;">
      ...
      <a href="https://man-woman-analysis-report.vercel.app/episodes/${episode.id}" style="display: inline-block; background-color: #111827; color: #ffffff; font-weight: 500; font-size: 15px; text-decoration: none; padding: 12px 24px; border-radius: 6px;">남녀 심리 가이드 읽기</a>
...
`;
    ...
    console.log('[SEND BATCH] All groups processed. Summary:', results);
